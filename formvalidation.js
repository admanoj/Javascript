...
function lnameValidate(){
    ...
    displayMsg("lasttname must be more than 2 characters", "lnameMsg","red")
    return false
}
else if(!last_name.match(/^([a-zA-Z])+$/)){
    displayMsg("lasttname contains only alphabets","lnameMsg","red")
    return false;
}
...
function passwordValidate(){
    ...
    displayMsg("must conatains 9 characters,One Uppercase,one Lowercase,one digit and one special character","passMsg","red")
    return false;
} 
...
function validform(){
    if(fnameValidate()&& lnameValidate()&&enameValidate()&& passwordValidate()){
        return true;
    }
    return false;
}
